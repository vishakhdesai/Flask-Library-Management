...
class BookSchema(schema.Schema):
    class Meta:
        model = Book
        fields = ("id", "bookID", "title", "authors", "average_rating", "isbn", "isbn13", "language_code", "num_pages", "ratings_count", "text_reviews_count", "publication_date", "publisher", "quantity")
...
def import_books():
    ...
    return render_template("import-books.html", books=books, title=title, authors=authors)
...
def get_books():
    ...
    title = request.args.get("title", "")
    authors = request.args.get("authors", "")
    return render_template("library-books.html", books=books, title=title, authors=authors, total_pages= total_pages)
