...
def issue_books(member_id):
    ...
    if(member.outstanding_debt >=500):
        return jsonify({"message": "Member's Outstanding Debt is more than ₹500"})
    books = []
    data = request.json
    for b in data["books"]:
...
def calculate_total_fee(rental_id):
    ...
    return jsonify({"total_fee": total_fee, "member_outstanding_debt": rental.member.outstanding_debt})
